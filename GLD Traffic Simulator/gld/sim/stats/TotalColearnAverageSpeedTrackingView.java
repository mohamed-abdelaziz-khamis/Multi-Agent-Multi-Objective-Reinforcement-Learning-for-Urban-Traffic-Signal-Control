...
/**
 *
 * TrackingView that tracks the colearn average speed of all roadusers that either have/haven't arrived yet.
 *
 * @author  EJUST
 * @version 1.0
 */

public class TotalColearnAverageSpeedTrackingView extends ExtendedTrackingView
{
	NodeStatistics[][] stats;
	DrivelaneSpeedStatistics[][] drivelaneSpeedStats;
	DrivelaneColearnStatistics[][] drivelaneColearnStats;
	
	public TotalColearnAverageSpeedTrackingView(int startTimeStep, SimModel model)
	{
		super(startTimeStep);
	...
	{ 
		...
		for(int i = 0; i < drivelaneSpeedStats.length; i++) {
			...
			distanceSample += (drivelaneSpeedStats[i][index].getAvgDistance(allTime)+
					   		   drivelaneColearnStats[i][index].getAvgExpectedDistance(allTime))* ru;
			
			timeSample += (drivelaneSpeedStats[i][index].getAvgTime(allTime)+
	...
	public String getDescription() { return "colearn average speed"; }
	
	protected String getYLabel() { return "colearn average speed (meter/timestep)"; }
}