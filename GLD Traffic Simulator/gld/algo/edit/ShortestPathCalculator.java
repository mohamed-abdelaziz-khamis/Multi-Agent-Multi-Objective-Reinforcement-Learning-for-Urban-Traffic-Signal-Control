...
/**
 * This class will determine for each node for each exit node what lanes are on a shortest path towards it.
 */

public class ShortestPathCalculator
{
    private final static int int_MAX = Integer.MAX_VALUE; 
    // To be used as 'infinite'
    
    private Node[] allNodes;
    private Node[] v;
    private int[] d;
    
    private final static double derivation_factor = 1.1; 
    // Factor of how much longer a path may be to be seen as a 'shortest path'
     ...
     * @param infra The Infrastructure of which the shortest paths should be calculated.
     * @param exit The Node to which the shortest paths should be calculated.
     */
    private void calcShortestPaths(Infrastructure infra, Node exit) throws InfraException
    {
        Node node = exit; 			
        // The Node variable we need for tracking
        
        int exitId = exit.getId();  
        // The ID we need for entering the found route in the SPData
        
        //int nearest = int_MAX; 		// Needed for speeding up the 'findNearest..'
        //int nearestId = 0;
        
        int id = exitId; 
        // The id of the node we're working with
        
        int num_nodes = infra.getAllNodes().length; 
        // Needed for looping through all nodes

        d = new int[num_nodes]; 
        // To keep track of the "length of the shortest path", necessary for this algorithm.

        // To get all the types of Roadusers that can reach this ExitNode
        Drivelane[] lanes = node.getAvailableInboundLanes(); 
        //GASTON: getInboundLanes --> getAvailableInboundLanes
        
      //Drivelane[] edgelanes = exit.getInboundLanes();
        
        Drivelane[] splanes; 
        // For looping, no need to recreate the Object all over
      ...
      //int num_edgelanes = edgelanes.length;
        ...
        for (int i = 0; i < num_lanes; i++)
        {
        	 ...
        	 * This is a combination of Roaduser types that may use this drivelane */
        }

        int[] type_arr = Typeutils.getTypes(types); 
        // Getting all primitive types of that type-integer
        ...
        //System.out.println("Number of in bound lanes on EdgeNode " + exit.getId() + " : " + num_lanes + " total type:" + types);
        ...
        for (int t = 0; t < num_types; t++) // For each of those Roaduser Types
        {
            ...
            v = (Node[]) allNodes.clone();
            // We need all the nodes as we will have to determine the shortest paths
            // for each node to the given exit.
            
            // We use an array as with using the Id as the key, 
            // we get an O(1) speed of resolving the actual Node-pointer
            ...
            while (num_nodes_left != 0)
            {
                ...
                id = findNearestAvailableNode(); 
                // Find the ID of the nearest Node not yet in our 'Cloud'. 
                // To help the search, supply a suspect.
                ...
                v[id] = null; // Remove that node from our 'to do' list

                splanes = node.getShortestPaths(exitId, type_arr[t]); 
                // Get the lanes that are on a shortest path from this node to the exitNode.
                ...
                {
                    lanes = node.getAvailableLanesLeadingTo(splanes[i], type_arr[t]); 
                    // Get an array of Drivelanes that can transfer Roadusers to the shortest-path lane
                    //GASTON: getLanesLeadingTo --> getAvailableLanesLeadingTo
                    ...
                    for (int j = 0; j < num_working_lanes; j++) // For each of those lanes
                    {
                        ...
                        {
                            lanelength = l.getLength();//Returns the length of the Road of this Drivelane 
                            newNode = l.getNodeComesFrom();
                            newId = newNode.getId();

                            if (d[id] + lanelength < d[newId]) 
                            	// Determine if the road to the Exitnode is quicker via node
                            { 
                            	// If so, Remove the earlier thought-up shortest paths, and use this one.
                                d[newId] = d[id] + lanelength; 
                                // Also update the newly found length of the shortest path.
                                //System.out.println("Set SP from: " + newId + " to " + exitId + " : " + (d[id]+lanelength) + " oldlength: "+d[newId]);
                            ...
                            else if (d[id] + lanelength == d[newId]) 
                            	// If the length of this found path is the same as the one already found,
                            { 
    ...
    /**
     * Find the nearest which can be chosen as an alternative destination
