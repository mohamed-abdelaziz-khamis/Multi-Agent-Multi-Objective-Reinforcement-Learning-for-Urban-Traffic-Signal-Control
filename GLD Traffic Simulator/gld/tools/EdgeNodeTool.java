	...
	*/
	...
	public void mouseReleased(View view, Point p, Tool.Mask mask)
	{
		if (modify != null && modify == selected) // currently dragging
		{
			...
			/*EJUST: 
			 * old scenario: modify.getSpawnFrequency(curRuType) = -1
			 * means that spawnFreq[i].ruType != ruType
			 * that leads to spawn = modify.getSpawnFrequency(curRuType) < 0
			 * 
			 * new scenario: modify.getSpawnFrequency(curRuType) = null 
			 * means that spawnFreq[i].ruType != ruType
			 */
			
			if (modify.getSpawnFrequency(curRuType) != null) {
				float spawn = modify.getSpawnFrequency(curRuType).freq /*EJUST add .freq*/;
				if (spawn!=-1){ /*EJUST*/
					spawn += freqChange;
						
					if(spawn > 1) spawn = (float)1.0; 
					/* EJUST: Not reasonable that the spawn be > 1 
					 * (cause this fixed spawn is compared to a generated uniform random value)
				  	 * 	and moreover practically its maximum is 0.4*/
			
					if(spawn < 0) spawn = (float)0.0;
					/* EJUST: This case is also unreasonable, moreover it will not happen cause 
					 * now the user can not input a negative value in any spawn textbox*/
				}	
				sm.setSpawnFrequency(modify, curRuType, spawn, 
						modify.getSpawnFrequency(curRuType).distributionType /*EJUST*/, 
						modify.getSpawnFrequency(curRuType).paramValue/*EJUST*/, 
						modify.getSpawnFrequency(curRuType).weatherCondition/*EJUST*/);
			}
		}
