 ...
 * @version 1.0
 */
public class TCACBsG extends TCRL implements Colearning, InstantiationAssistant, HECinterface, Constants /*EJUST*/
{
    // TLC vars
    		...
    		PosMov[] posMovs, Drivelane desired, int penalty)
    {
             ...
             signnow.getType() == Sign.NO_SIGN))
        {
            ...
            recalcP(tlId, prevpos_int, desId, prevsign.mayDrive(), signnow.getId(), posnow_int, isAccidentInt, isCongestedInt);
            recalcQ(tlId, prevpos, desId, prevsign.mayDrive(), signnow.getId(), posnow, posMovs, isAccidentInt, 
            			isCongestedInt, prevlane, ru, penalty);
            recalcV(tlId, prevpos_int, desId, isAccidentInt, isCongestedInt);
        }
    }
                           ...
                           int tlNewId, int posNew, int isAccident, int isCongested)
    {
        ...
        CountEntry thisSituation = new CountEntry(tlId, pos, desId, light, tlNewId, posNew, isAccident, isCongested);
        int c_index = count[tlId][pos][desId][isAccident][isCongested].indexOf(thisSituation);
        if (c_index >= 0)
        {
            // Entry found
            thisSituation = (CountEntry) count[tlId][pos][desId][isAccident][isCongested].elementAt(c_index);
        ...
        {
            curC = (CountEntry) count[tlId][pos][desId][isAccident][isCongested].elementAt(i);
        ...
        PEntry thisChance = new PEntry(tlId, pos, desId, light, tlNewId, posNew,isAccident,isCongested);
        int p_index = pTable[tlId][pos][desId][isAccident][isCongested].indexOf(thisChance);

        if (p_index >= 0)
        {
            thisChance = (PEntry) pTable[tlId][pos][desId][isAccident][isCongested].elementAt(p_index);
        }
        else
        {
    ...
    protected void recalcQ(int tlId, double pos /*EJUST: int --> double*/, 
    					   ...
    					   PosMov[] posMovs, int isAccident, int isCongested, Drivelane dlnow,
                           Roaduser ru, int penalty)
    {
        ...
        { 
            ...
            if (curPMTlId != tlId)
            { // In case RU just crossed a junction, a next sign has to be determened.
            ...
            PEntry P = new PEntry(tlId, pos_int, desId, light, curPMTlId, curPMPos, isAccident, isCongested);
            int p_index = pTable[tlId][pos_int][desId][isAccident][isCongested].indexOf(P);

            if (p_index >= 0)
            {
                P = (PEntry) pTable[tlId][pos_int][desId][isAccident][isCongested].elementAt(
                        p_index);
                R = rewardFunction(tlId, pos, curPMTlId, curPosMov.pos, isAccident, isCongested);
        ...
        qTable[tlId][pos_int][desId][isAccident][isCongested][light ? green_index : red_index] = Q;
    }
    ...
    { //  V([tl,p,d,isAccident,isCongested]) = Sum (L) [P(L|(tl,p,d,isAccident,isCongested))Q([tl,p,d,isAccident,isCongested],L)] ??
        ...
        vTable[tlId][pos][desId][isAccident][isCongested] = (pGreen * qGreen) + (pRed * qRed);
    }
    ...
    protected float[] calcPGR(int tlId, int pos, int desId, int isAccident, int isCongested)
    {
        ...
        for (; psize >= 0; psize--)
        {
            PEntry cur = (PEntry) pTable[tlId][pos][desId][isAccident][isCongested].elementAt(psize);
            if (cur.light == green)
            {
    ...
    public Object createInstance(Class request) throws ClassNotFoundException, InstantiationException, IllegalAccessException
    {
        ...
        {
            throw new ClassNotFoundException ("TCACBsG IntstantiationAssistant cannot make instances of " + request);
        }
    }
}
