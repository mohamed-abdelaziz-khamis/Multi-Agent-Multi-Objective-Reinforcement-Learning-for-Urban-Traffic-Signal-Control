...
public class MorevtsSotlPlatoon2 extends TLController {
	...
	public final static String shortXMLName="Sotl-Platoon2";
	
	public MorevtsSotlPlatoon2(Infrastructure infra) 
	{	
		super(infra);
	...
	public int countRoadusers(Drivelane lane, double range /*EJUST: int --> double*/) {
		...
		while (li.hasNext() && !stop) {
			...
			if (ru.getPosition() <= range) 
				cntr += ru.getPlatoonValue(); 
				/* EJUST comment: This is the only difference between MorevtsSotlPlatoon and MorevtsSotlPlatoo2
				 * ru.getPlatoonValue() seems like ru.getNumPassengers() that gives a weight for each roaduser type.*/
			else 
				stop = true;
		}
		return cntr;
	...
	public void updateTLDs() {
		...
		for (int i = 0; i < tld.length; i++) { // for all nodes
			...
			for (int j = 0; j < tld[i].length; j++)
				// if tl is green
				if (tld[i][j].getTL().getState()) {
					platoonRuCntTmp = countRoadusers(tld[i][j].getTL().getLane(),OMEGA);
					platoonRoaduserCount += platoonRuCntTmp;
					if(platoonRuCntTmp > 0)	
						nrOfPlatoonLanes += 1;
					else 
						emptyLanes += 1;
				}
			
			if (nrOfPlatoonLanes==0 && emptyLanes>0) 
				nrOfPlatoonLanes = 1;
...
/*	*//** 
	...
	*//**
	 ...
	 *//*	
	public TLDecision[][] decideTLs()
	{	// System.out.println("Current timeStep = "+
		// (getInfrastructure().getCurTimeStep()));
		TLDecision curDec;
		Drivelane curLane;
