 ...
 * @version 2.0
 */
public class TC1TLCDestless extends TCRL implements Colearning, InstantiationAssistant, Constants /*EJUST*/
{
	// TLC vars
	...
	*/
		...
		for (int i=0;i<num_nodes;i++) {
			...
			for(int j=0;j<num_dec;j++) {
				...
				for(int k=0; k<waitingsize; k++) {
					...
					desId = 0; //EJUST comment: Instead of ru.getDestNode().getId();
			...
			PosMov[] posMovs, Drivelane desired, int penalty)
	{
		...
		if(prevsign.getType()==Sign.TRAFFICLIGHT && (signnow.getType()==Sign.TRAFFICLIGHT || signnow.getType()==Sign.NO_SIGN)) {
			...
			int desId = 0; //EJUST comment: Instead of ru.getDestNode().getId();
	...
	*/
	...
	protected int rewardFunction(int tlId, double pos /*EJUST: int --> double*/, int tlNewId, double posNew /*EJUST: int --> double*/) {
