...
public abstract class TrackingView extends Canvas implements Observer
{
	...
	/** The timeStep this view started tracking */
	protected int startTimeStep;
	/** The timeStep the end of the x-axis currently represents */
	protected int endTimeStep;
	...
	/** String containing highlighted timeStep text */
	protected String highlightTimeStep;

	/** Current index in trackData[][]. */
	protected int curIndex;
	/** Highest valued sampled. */
	protected float maximum;
	/** Sample rate: each trackRate timeSteps, a sample is taken. */
	 ...
	 *
	 * @param _startTimeStep The timeStep this view starts tracking.
	 */
	public TrackingView(int _startTimeStep)
	{
	...
	protected void init(int _startTimeStep)
	{
		...
		curIndex = 0;
		maximum = (float)5.0;
		endTimeStep = startTimeStep + MAX_DATA;
		highlightIndex = 0;
	}
	...
	/** Returns the timeStep this window started tracking */
	public int getStartTimeStep() { return startTimeStep; }
	/** Sets the timeStep this window started tracking */
	public void setStartTimeStep(int timeStep) { startTimeStep = timeStep; }
	...
	public void drawBuffer()
	{
		...
		// x-axis
		g.drawString(startTimeStep + "", 30, h - 7);
		g.drawString(endTimeStep + "", w - 40, h - 7);
		g.drawString("time (time steps)", w / 2 - 20, h - 5);
		...
		if(highlightIndex != 0)
		{
			...
			g.setColor(Color.black);
			g.drawString(highlightTimeStep, w - 80, 15);
			int y = 35;
			for(int num=0; num<MAX_TRACK; num++)
				if(show[num]) {
					g.setColor(colors[num]);
	...
	*/
	protected void collapseData()
	{
		...
		curIndex = MAX_DATA / 2;
		trackRate *= 2;
		endTimeStep = startTimeStep + (endTimeStep - startTimeStep) * 2;
		highlightIndex = 0;
	}
	...
	public void saveData(String filename, SimModel model) throws IOException
	{
		...
		out.println("# Format: <TimeStep>" + SEP + fieldsDesc); out.println("#");

		String ln;
		for(int i=0; i<curIndex; i++) {
			ln = (startTimeStep + i * trackRate) + SEP;
	...
	protected class MyMouseListener extends MouseAdapter
	{
		public void mouseClicked(MouseEvent e)
		{
			...
			else
			{
				...
				highlightTimeStep = "step = " + (startTimeStep + highlightIndex * trackRate);
				String desc;
				for(int i=0; i<MAX_TRACK; i++) {
					highlightText[i] = getSourceDesc(i) + " = " + trackData[i][highlightIndex];
					int dot;
					if((dot = highlightText[i].indexOf(".")) > -1)
						highlightText[i] = highlightText[i].substring(0,dot+2 /*EJUST: 2 --> 3*/);
				}
			}
			redraw();
		}
