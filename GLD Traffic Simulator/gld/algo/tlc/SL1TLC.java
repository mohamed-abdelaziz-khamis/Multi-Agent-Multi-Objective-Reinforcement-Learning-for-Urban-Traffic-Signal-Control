 ...
 * @version 1.1
 */
public class SL1TLC extends TCRL implements InstantiationAssistant, Constants /*EJUST*/
{   
    ...
    */
    ...
    protected void recalcQ(Sign tl, double pos /*EJUST: int --> double*/, 
    						Node destination, boolean light, Sign tl_new, double pos_new /*EJUST: int --> double*/, 
    						boolean light_new, PosMov[] posMovs, int penalty)
    {
        ...
        */
        ...
        /*EJUST*/
        int pos_new_int = (int) Math.ceil(pos_new);
        int pos_int = (int) Math.ceil(pos);

        try {
          oldQvalue = q_table[tl.getId()][pos_int][destination.getId()][light?green_index:red_index];
          Qmark = q_table[tl_new.getId()][pos_new_int][destination.getId()][light_new?green_index:red_index];// Q( [ tl' , p' ] , L')
        }
        catch (Exception e) {
        ...
        q_table[tl.getId()][pos_int][destination.getId()][light?green_index:red_index]=newQvalue;
    }
    ...
    */
    ...
    protected int rewardFunction(Sign tl_new, double pos_new /*EJUST: int --> double*/, PosMov[] posMovs)
    {
        ...
        for(int i=0; i<size; i++) {
            if(posMovs[i].sign.getId() /*EJUST: tlId*/ == tl_new.getId() && Math.abs(posMovs[i].pos-pos_new) <= STOP_SPEED_MS*TIMESTEP_S /*EJUST replaced: pos==pos_new*/)
                return 1; 
            /*EJUST: 0 --> 1 in order to comply with the decision making mechanism 
                that chooses the gain of the maximum accumulated penalty 
                (usually with the maximum number of waiting road users)*/
        }
    ...
    */
    ...
    public class CountEntry implements XMLSerializable , TwoStageLoader
    {
        ...
        { 
          Dictionary laneDictionary=(Dictionary)(dictionaries.get("lane")), nodeDictionary=(Dictionary)(dictionaries.get("node"));
...
//OBS       // Her burde laves et tjek p� om 0 < gamma < 1 og det samme med random chance
    ...
    {   
        ...
        { 
        	throw new ClassNotFoundException("SL1 IntstantiationAssistant cannot make instances of "+ request);
        }
    }
}
