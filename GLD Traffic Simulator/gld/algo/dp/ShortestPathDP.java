...
public class ShortestPathDP extends DrivingPolicy
{
	...
	{		
		...
		{	
			//GASTON: 
			//The original code was returning null. 
			//After the modifications if we return null, all the cars in this lane will be erased.
			//There is some code (commented) that try to find an alternative path, but it needs additional testing
			
			/*
			if (shortest.length != 0)
			{
				for (int h=0; h < shortest.length; h++)
				{
					Road road = shortest[h].getRoad();
					if (road.isEnabled()) 
						return shortest[h];
				}
			}
			...
			if (allOutgoing.length != 0)
			{
				for (int h=0; h < shortest.length; h++)
				{
					Road road = allOutgoing[h].getRoad();
					if (road.isEnabled()) 
						return allOutgoing[h];
				}
			}*/
