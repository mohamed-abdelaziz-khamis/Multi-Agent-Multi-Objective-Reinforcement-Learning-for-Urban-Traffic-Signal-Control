...
public class GenNeuralTLC extends TLController implements XMLSerializable,TwoStageLoader,InstantiationAssistant, Constants /*EJUST*/
{
	...
	protected double num_wait; /*EJUST: int --> double*/
	/** counter for the number of roadusers that did move */
	protected double num_move; /*EJUST: int --> double*/
		...
		for(int i=0;i<num_nodes;i++) {
			...
			for(int j=0;j<num_tl;j++) {
				...
				if(i==trackNode) {
					...
					System.out.println("node: "+i+" light: "+j+" gain: "+q+" "+
	...
	/**
	 * Provides the TLC-algorithm with information about a roaduser that has had it's go in the moveAllRoaduser-timeStep.
	...
	public void updateRoaduserMove(Roaduser _ru, Drivelane _prevlane, Sign _prevsign, double _prevpos /*EJUST: int --> double*/, 
			...
			PosMov[] posMovs, Drivelane _desired)
	{
		// Should keep track of waits and moves per Node
		if(_prevsign == _signnow && (_prevpos-_posnow)<=STOP_SPEED_MS*dt /*EJUST replaced: _prevpos==_posnow */) {
			...
			num_wait += _ru.getSpeed()*dt;
			ind[nodeId].setWait(ind[nodeId].getWait()+_ru.getSpeed()*dt);
		}
		else {
			// Roaduser did move
			// Find the Node where ru moved:
			int nodeId = _prevlane.getNodeLeadsTo().getId();
			ind[nodeId].setMove(ind[nodeId].getMove()+_ru.getSpeed()*dt);
		}
		...
		if(_prevsign == _signnow && (_prevpos-_posnow)<=STOP_SPEED_MS*dt /*EJUST replaced: _prevpos==_posnow */) {
			...
			double old_move = num_move; /*EJUST: int --> double*/
			double thesemoves = (_prevpos - _posnow); /*EJUST: int --> double*/
			double thesewaits = _ru.getSpeed()*dt - (_prevpos - _posnow); /*EJUST: int --> double*/
			int nodeId = _prevlane.getNodeLeadsTo().getId();
	...
	{	
		...
		protected double wait; /*EJUST: int --> double*/
		protected double move;	/*EJUST: int --> double*/
			...
			try {
				...
				for(int i=0;i<num_nodes;i++) {
					...
					for(int j=0;j<num_lanes;j++) {
						Drivelane d;
						input_set[input_index] = 
							(float) tld[thisNodeId][j].getTL().getLane().getNumBlocksWaiting() / 
							(float) tld[thisNodeId][j].getTL().getLane().getLength();
						input_index++;
					}
				}
		 ...
		 * @param _wait
		 */
		protected void setWait(double _wait) { /*EJUST: int --> double*/
			wait = _wait;
		}
		 ...
		 */
		protected double getWait() { /*EJUST: int --> double*/
			return wait;
		}
		 ...
		 * @param _move
		 */
		protected void setMove(double _move) { /*EJUST: int --> double*/
			move = _move;
		}
		 ...
		 */
		protected double getMove() { /*EJUST: int --> double*/
			return move;
		}
	...
	{ 	
		...
		{ 
			throw new ClassNotFoundException ("GenNeural InstantiationAssistant cannot make instances of "+ request);
		}
	}

	public boolean canCreateInstance (Class request)
	{ 	
		return GenNeuralPopulation.class.equals(request) || GenNeuralIndividual.class.equals(request) ;
	}
