...
public class CurveUtils
{
   ...
   */
	public static Point getPoint(TurnCurve c, double index/*EJUST: int --> double*/, int step) 
	{
		if(c.isStraight())
		{
			...
			Point p = new Point((int)((double)c.getFirst().x + dx * index),(int)((double)c.getFirst().y + dy * index));
			if(c.getFirst().distance(p) > c.getFirst().distance(c.getLast())) return c.getLast();
			if(c.getLast().distance(p) > c.getLast().distance(c.getFirst())) return c.getFirst();			
			return p;
		}
		Point p;
		Point q = c.getFirst();
    ...
    */
    public static double getAngle(TurnCurve c, double index /*EJUST: int --> double*/, int step)
    {
		Point p1;
		Point p2;
		if(c.isStraight())
	...
	public static void setupRoadSizes(Road road, int step) throws InfraException
	{
		...
		int alphaMaxWidth = alphaNode.getWidth()*10;
		int betaMaxWidth = betaNode.getWidth()*10;
