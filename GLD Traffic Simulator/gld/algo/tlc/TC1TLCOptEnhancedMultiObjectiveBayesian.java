 ...
 * @version 2.0
 */
public class TC1TLCOptEnhancedMultiObjectiveBayesian extends TCColearnPerformanceIndex implements Colearning, InstantiationAssistant, HECinterface, Constants /*EJUST*/
{
	// TLC vars
	...
	protected final static String shortXMLName="tlc-tc1o1-enhanced-multiobjective-bayesian";
	...
	public TC1TLCOptEnhancedMultiObjectiveBayesian ( Infrastructure infra ) throws InfraException
	{	
		super(infra);
					...
					for (int j=0; j<dls.length; j++) 
					{
						...
						for (int k=0; k<num_pos_on_dl; k++)	
						{
							...
							for (int l=0; l<num_specialnodes;l++)	
							{
								...
								qTableMultiObjective[id][k][l][1]= 10.0f; //EJUST ICMLA 2012 version: 10.0f instead of 0.0f 
								...
								vTableMultiObjective[id][k][l]	= 5.0f; //EJUST ICMLA 2012 version: 5.0f instead of 0.0f								
							...
							double posNew /*EJUST: int --> double*/)
	{
		...
		try { /*POMDPGLD*/			
			...
			System.err.print("ArrayIndexOutOfBoundsException in: TC1TLCOptEnhancedMultiObjectiveBayesian, r: 504, values: tlId: "+tlId+" pos: "+pos+" desId: "+desId);
			c_index = 0;
		}/*POMDPGLD*/
		...
		if (prevsign.getLane().isPrimary()){ //Primary lane, i.e., main road
			r_AverageTripTime = 10*Math.pow(2, -r_AverageSpeed);
			...
			r_AverageTripTime = 10*Math.pow(2, -Math.pow(r_AverageSpeed,2));
	...
	{						
		...
		return 10; //tlId==tlNewId and pos-posNew <= STOP_SPEED_MS*TIMESTEP_S /*L = Red*/		
	...
	{
		if(b) c.setStatus("Using HEC on TC1TLCOptEnhancedMultiObjectiveBayesian");
		else c.setStatus("Using TC1TLCOptEnhancedMultiObjectiveBayesian without HEC");
		hecAddon = b;
	}
	...
	{ 	
		System.out.println("Called TC1TLC-opt Enhanced Multi-objective Bayesian instantiation assistant ??");
	...
	{ 	
		...
		{ 
			throw new ClassNotFoundException("TC1 Enhanced Multi-objective Bayesian IntstantiationAssistant cannot make instances of "+ request);
		}
	}
}