 ...
 * @version 1.0
 */
public class TC2Final extends TCRL implements Colearning,InstantiationAssistant, Constants /*EJUST*/
{
	// TLC vars
	...
	*/
								...
								PosMov[] posMovs, Drivelane desired, int penalty)
	{
		...
		if(prevsign.getType()==Sign.TRAFFICLIGHT && (signnow.getType()==Sign.TRAFFICLIGHT || signnow.getType()==Sign.NO_SIGN)) {
			...
			recalcQ(tlId, prevpos_int, desId, prevsign.mayDrive(), signnow, posnow_int, posMovs, Ktl, ru, isWaiting, penalty);
...
//				R = calcReward(tlId,pos,tlNewId,t);
	...
	*/
	...
	protected int calcReward(int tlId, double pos /*EJUST: int --> double*/, int tlNewId, double posNew /*EJUST: int --> double*/) {
