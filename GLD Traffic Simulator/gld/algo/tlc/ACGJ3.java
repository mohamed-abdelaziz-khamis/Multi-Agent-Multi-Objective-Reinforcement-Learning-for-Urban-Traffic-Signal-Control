...
public class ACGJ3 extends TLController implements InstantiationAssistant,XMLSerializable, Constants /*EJUST*/
{
	...
	/** counter for the number of nodes roadusers that did move */
			...
			for(int j=0;j<num_tl;j++) {
				...
				if(i==trackNode) {
					...
					System.out.println("node: "+i+" light: "+j+" gain: "+q+" "+
							targets[0]+" "+targets[1]+" "+targets[2]+" "+currentlane.getNumRoadusersWaiting());
				}
				if(q==Float.NaN) {
	...
	protected class ACGJ3Population implements XMLSerializable
	{
		 ...
		 */
		protected ACGJ3Individual getNextIndividual(double wait, double move) { /*EJUST: int --> double*/
		...
		protected void evolve()
		{
			if(this_gen<num_gen) {
				...
				for(int i=0;i<num_ind;i++) {
					...
					if(fit>maxfit)
						maxfit = fit;
					double wait = inds[i].getWait(); /*EJUST: int --> double*/
					double move = inds[i].getMove(); /*EJUST: int --> double*/
					totwait+=wait;
					totmove+=move;
				}
