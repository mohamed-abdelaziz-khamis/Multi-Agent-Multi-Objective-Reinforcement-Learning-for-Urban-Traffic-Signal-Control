 ...
 * @version 1.1
 */
public class SL3TLC extends TCRL implements InstantiationAssistant, Constants /*EJUST*/
{   
    ...
    */
    		...
    		PosMov[] posMovs, Drivelane desired, int penalty)
    {
        ...
        if(prevsign.getType()==Sign.TRAFFICLIGHT && 
            ...
            recalcQ(prevsign, prevpos, dest, prevsign.mayDrive(), signnow, posnow, 
            		signnow.mayDrive(), posMovs, penalty);
        }
    }
    ...
    protected int rewardFunction(Sign tl_new, double pos_new /*EJUST: int--> double*/, PosMov[] posMovs)
    {
        ...
        for(int i=0; i<size; i++) {
            if(posMovs[i].sign.getId() /*EJUST: tlId --> sign.getId() */ == tl_new.getId() 
            		&& Math.abs(posMovs[i].pos-pos_new) <= STOP_SPEED_MS*TIMESTEP_S /*EJUST replaced: pos==pos_new*/)
                return 0;
        }
...
//OBS       // Her burde laves et tjek p� om 0 < gamma < 1 og det samme med random chance
    ...
    {   
        ...
        { 
        	throw new ClassNotFoundException("SL3 IntstantiationAssistant cannot make instances of "+ request);
        }
    }
}