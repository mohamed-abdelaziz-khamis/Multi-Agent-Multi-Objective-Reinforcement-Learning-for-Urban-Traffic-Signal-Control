 ...
 * @version 1.0
 */
public class TC3B1 extends TCRL implements Colearning,InstantiationAssistant, Constants /*EJUST*/
{
	// TLC vars
	...
	*/
			...
			PosMov[] posMovs, Drivelane desired, int penalty)
	{
		...
		if(prevsign.getType()==Sign.TRAFFICLIGHT && (signnow.getType()==Sign.TRAFFICLIGHT || signnow.getType()==Sign.NO_SIGN)) {
			...
			recalcP(prevsign, prevpos_int, dest, light, signnow, posnow_int, Ktl);
			recalcVa(prevsign, prevpos_int, dest);
			recalcV(prevsign, prevpos_int, dest, light, Ktl);
			recalcQa(prevsign, prevpos, dest, light, signnow, posnow, posMovs, penalty);
			recalcQ(prevsign, prevpos_int, dest, light, signnow, posnow_int, posMovs, Ktl, penalty);
		}

		if(prevsign == signnow && prevpos-posnow <= STOP_SPEED_MS*TIMESTEP_S /*EJUST replaced: prevpos == posnow*/) {
			...
			relaxBucket(prevlane, prevpos_int);
		}
	}
	...
	protected void recalcQa(Sign tl, double pos /*EJUST: int --> double*/, 
			...
			PosMov[] posMovs, int penalty)
	{
		...
		for(; size>=0; size--) {
			PEntry P = new PEntry(tl, pos_int, destination, light, tl, size);

			int p_index = p_table[tlId][pos_int][desId].indexOf(P);
			if(p_index>=0) {
				try {
					P = (PEntry) p_table[tlId][pos_int][desId].elementAt(p_index);
		...
		try {
			qa_table[tl.getId()][pos_int][destination.getId()][light?green_index:red_index] = newQvalue;
		}
		catch (Exception e) {
	...
	*/
	...
	protected int rewardFunction(Sign tl_new, double pos_new /*EJUST: int --> double*/, PosMov[] posMovs)
	{
		...
		for(int i=0; i<size; i++)
			if( posMovs[i].sign.getId() /*EJUST: tlId --> sign.getId() */==tl_new.getId())
				if(Math.abs(posMovs[i].pos-pos_new) > STOP_SPEED_MS*TIMESTEP_S /*EJUST replaced: posMovs[i].pos != pos_new*/)
						return 0;
		return 1;
	}
