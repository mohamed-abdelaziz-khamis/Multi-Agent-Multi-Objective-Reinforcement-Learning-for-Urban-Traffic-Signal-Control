...
public class ACGJ5 extends TLController implements InstantiationAssistant,XMLSerializable, Constants /*EJUST*/
{
	    		...
	    		if(trackNode!=-1)
					if(i==trackNode) {
						...
						System.out.println("N:"+i+" L:"+j+" G:"+gain+" <:"+
								...
								" W:"+currentlane.getNumRoadusersWaiting());
					}
			...
			PosMov[] posMovs, Drivelane _desired)
	{
		if(_prevsign == _signnow && (_prevpos-_posnow)<=STOP_SPEED_MS*dt /*EJUST replaced: _prevpos==_posnow */) {
			...
			if(_prevsign != _signnow) {
				...
				num_wait += _ru.getSpeed()*dt - (_prevpos - _posnow);
			}
		}
	}
	...
	protected class Population implements XMLSerializable
	{
		 ...
		 */
		protected Individual getNextIndividual(double wait, double move) { /*EJUST: int --> double*/
		...
		{	
			if(this_gen < NUMBER_GENERATIONS) {
				...
				System.out.println("Stats of prev. gen: (a-wait,a-move)=("+avg_wait+","+avg_move+")"+
						" (a-fit,mx-fit)=("+avg_fit+","+max_fit+")");
	...
	{	
		...
		protected void setWait(double _wait)		{	wait = _wait;		} /*EJUST: int --> double*/
		/* Returns the number of Roadusers I caused to wait */
		protected double getWait() 				{	return wait;		} /*EJUST: int --> double*/
		/* Sets the number of Roadusers this Individual made moving */
		protected void setMove(double _move)		{	move = _move;		} /*EJUST: int --> double*/
		/* Returns the number of Roadusers I caused to move */
		protected double getMove() 				{	return move;		} /*EJUST: int --> double*/
	...
	{
		String[] descs = {"# of steps an individual may run", 
	...
	{ 	
		...
		result.addAttribute(new XMLAttribute("ind-index", StringUtils.getIndexObject(pop.inds,ind)));
	  	return result;
	}
	...
	{ 	
		...
		{ 
			throw new ClassNotFoundException ("ACGJ5 InstantiationAssistant cannot make instances of "+  request);
		}
	}

	public boolean canCreateInstance (Class request)
	{ 	
		return Population.class.equals(request) || Individual.class.equals(request);
	}
}
