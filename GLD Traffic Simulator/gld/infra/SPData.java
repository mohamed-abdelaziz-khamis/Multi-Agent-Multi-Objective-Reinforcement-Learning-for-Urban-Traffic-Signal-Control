 ...
 * 
 * GASTON: The exit nodes are the Edge Nodes that can be reached from this node. 
 * There will be one element in the Vector paths per exit node that can be reached 
 * (depending on the direction of the lanes an exit node can or cannot be reached)
 * 
...
public class SPData implements XMLSerializable,TwoStageLoader,InstantiationAssistant
{
	...
	public Drivelane[] getShortestPaths(int exitNodeId, int ruType) {
		...
		//GASTON: SE AGREGO EL try-catch para capturar un NullPointerException
		//Add the try-catch to catch a NullPointerException
		//try {
		...
		//} catch (Exception e) {
				//	System.out.println("Se capturo la excepcion");
				//	e.printStackTrace();
				//	return new Drivelane[0];
		//}
	}
	...
	/** GASTON: AGREGO ESTE METHOD PARA INTENTAR BORRAR TODOS LOS PATHS QUE TIENE UN NODO, DE MODO QUE EL SHORTESTPATHCALCULATOR LOS CALCULE TODOS DE NUEVO
	 * 
	 * Add this METHOD FOR ATTEMPTING TO DELETE ALL PATHS THAT HAS A NODE, SO THAT THE CALCULATED SHORTESTPATHCALCULATOR ALL NEW
	 *  */
	...
	{	
		...
		{ 
			throw new ClassNotFoundException ("SPData IntstantiationAssistant cannot make instances of "+  request);
		}
	}
 		...
 		{ 	
			Dictionary laneDictionary=(Dictionary)(dictionaries.get("lane"));
		  	...
		  	for (int t=0;t<loadData.laneIds.length;t++)
		      	lanes[t]=(Drivelane)(laneDictionary.get(new Integer(loadData.laneIds[t])));		
 		}		 
