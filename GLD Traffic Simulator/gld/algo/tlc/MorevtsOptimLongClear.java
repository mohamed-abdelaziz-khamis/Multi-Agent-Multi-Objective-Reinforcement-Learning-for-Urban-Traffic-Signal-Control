...
public class MorevtsOptimLongClear extends TLController implements Constants /*EJUST*/ {

	public int PERIOD = 60 ;
	
	public int CLEARTIME = 3;
	public int clearCntr;
	
	public final static String shortXMLName="Green Waves (Optim) long clear";
	
	/** EJUST: Position update time step*/
	protected static final double dt = TIMESTEP_S;
	
	public MorevtsOptimLongClear(Infrastructure infra) 
	{	
		super(infra);
		//setPhaseDiffs();
	}
			...
			for (int j = 0; j < tld[i].length; j++) { // for all inbound lanes in node
				...
				try {
					...
					if (curLane.isPrimary()) {
						...
						if (phase <= (PERIOD / 4)-CLEARTIME) {
							curDec.getTL().getNode().setAllRed(false);
					...
					else // current lane is non-primary
					{
						...
						else if (phase <= ((3 * PERIOD) / 4)-CLEARTIME) {
							curDec.getTL().getNode().setAllRed(false);
							if (curLane.getTarget(0))
								curDec.setGain(5);
							if (curLane.getTarget(1) || curLane.getTarget(2))
						...
						else if (phase <= PERIOD-CLEARTIME) {
							curDec.getTL().getNode().setAllRed(false);
							if (curLane.getTarget(0))
								curDec.setGain(0);
							if (curLane.getTarget(1) || curLane.getTarget(2))
