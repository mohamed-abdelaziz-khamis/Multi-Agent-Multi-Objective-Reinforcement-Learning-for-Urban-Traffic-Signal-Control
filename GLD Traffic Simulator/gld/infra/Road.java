...
public class Road implements Selectable, XMLSerializable, TwoStageLoader
{
	...
	/** Container object to transfer information from the first stage
	...
	/** (SBC) Maximum allowed speed at this road */
	protected double speed_max_allowed = 14; /*EJUST: int-->double*/

	/** (SBC) is this road an primary road or not */
	protected boolean primary = false;
	
	/**GASTON: shows whether traffic is allowed or not
	 */
	...
	/*POMDPGLD*/
    /** (RM 06) function to convert drivelanes to other types through the menu */
    public void convertLanes(Model m) throws InfraException {
        Drivelane[] alpha = getAlphaLanes();

        for (int i = 0; i < alpha.length; i++)
        {
            alpha[i] = new DrivelaneFactory(m,this).convertDrivelane(alpha[i]);
            Sign targetSign = alpha[i].getSign();
            if(targetSign != null) {
                targetSign.setLane(alpha[i]);
            }

        }
        this.setAlphaLanes(alpha);

        Drivelane[] beta = getBetaLanes();
        for (int i = 0; i < beta.length; i++)
        {
            beta[i] = new DrivelaneFactory(m,this).convertDrivelane(beta[i]);
            Sign targetSign = beta[i].getSign();
            if(targetSign != null) {
                targetSign.setLane(beta[i]);
            }
        }
        this.setBetaLanes(beta);
    }
	...
	/*============================================*/
 	...
 	{ 	
		...
		//GDE: these attributes are not required. This is to achieve compatibility with older versions of the product
	 	try {
			speed_max_allowed=myElement.getAttribute("speed-max-allowed").getDoubleValue(); // SBC, EJUST: int --> double
			primary=myElement.getAttribute("primary").getBoolValue(); // SBC
		} catch (NoSuchElementException e) {
			//GDE: we do nothing here, just keep it compatible
		}	
 	...
 	{ 	
 		XMLElement result=new XMLElement("road");
   		...
   		result.addAttribute(new XMLAttribute("speed-max-allowed",speed_max_allowed)); // SBC
   		result.addAttribute(new XMLAttribute("primary",primary)); // SBC
 	...
 	{ 	
		...
		XMLArray.saveArray (getDrivelaneIdList(alphaLanes),this,saver, "alpha-lanes");
		XMLArray.saveArray (getDrivelaneIdList(betaLanes),this,saver, "beta-lanes");
	}
...
*/
	...
	//SBC
	/** Returns the maximum allowed speed on this road */
	public double getSpeedMaxAllowed() { return speed_max_allowed; } /*EJUST: int-->double*/
	/** Sets the maximum allowed speed on this road */
	public void setSpeedMaxAllowed(double newSpeed) { speed_max_allowed = newSpeed; }  /*EJUST: int-->double*/
	
	/** Returns true if road is primary */
	public boolean isPrimary() { return primary; }
	/** Sets the primary property of this road */
	public void setPrimary(boolean art) { primary = art; }
	 ...
	 */
 	public void paintRoaduser(Graphics g, Roaduser r, Point p1, Point p2, double a1, double a2, double pos /*EJUST: int --> double*/)
  	{
  		try
  		{
...
//			double a = ((Double)o[1]).doubleValue();
			...
			Point p = CurveUtils.getPoint(arc,pos,Infrastructure.blockLength); //SBC: 10
			double a = CurveUtils.getAngle(arc,pos,Infrastructure.blockLength);//SBC: 10
	 ...
	 */
	public void paintRoaduserOnNode(Graphics g, Roaduser ru, Node node, Point p, double pos /*EJUST: int-->double*/, Drivelane d)
	{
		double na = 0.0;
		try
		{
			int id = ru.getPrevSign();
			/* Note that setting the prevSign before or after calling the update_Q_Values will not differ
			 * as this method is only useful in case of the reinforcement learning based traffic controllers
			 * while the method of paintRoaduserOnNode is a general method used by the simulator*/
	      ...
	      if(turns.length > 0 && !vertical)
	      {
			...
			for(int n = 0; n < alphaLanes.length; n++)
			{
				...
				if(queue.size() > 0)
				{
					...
					while(li.hasNext())
					{
						...
						double pos = r.getPosition(); /*EJUST: int --> double*/
							...
							for(int t = 0; t <= turns.length; t++)
							{
								if(t < turns.length && pos < length)
								{
									if(turns[t].getPosition() >= pos)
									{
										Point tp1, tp2;
										double ta1, ta2;
										double cpos = pos; /*EJUST: int --> double*/
										...
										else
										{
											...
											ta2 = turns[t].getAngle() + Math.PI;
											cpos = pos - turns[t-1].getPosition(); /*EJUST: remove casting to int*/
										}
										Point p1 = avg(getCurvePoint(tp1,ta1,-1,n,x,y), getCurvePoint(tp1,ta1,-1,(n+1),x,y));
										Point p2 = avg(getCurvePoint(tp2,ta2,1,n,x,y), getCurvePoint(tp2,ta2,1,(n+1),x,y));
								...
								else if(pos < length)
								{
									...
									double cpos = pos - turns[t-1].getPosition() /*EJUST: remove casting to int*/ + 1; /*EJUST: int --> double*/
									Point p1 = avg(getCurvePoint(tp1,ta1,-1,n,x,y), getCurvePoint(tp1,ta1,-1,n+1,x,y));
									Point p2 = avg(getCurvePoint(tp2,ta2,-1,n,x,y), getCurvePoint(tp2,ta2,-1,n+1,x,y));
									paintRoaduser(g,r,p1,p2,ta1,ta2,cpos);
								}
								...
								{
									Point tp1 = new Point((int)(betaCoord.x - 5 * Math.cos(betaAngle)),(int)(betaCoord.y + 5 * Math.sin(betaAngle)));
									Point p = avg(getCurvePoint(tp1,betaAngle,-1,n,x,y), getCurvePoint(tp1,betaAngle,-1,n+1,x,y));
			...
			for(int n = 0; n < betaLanes.length; n++)
			{
				...
				if(queue.size() > 0)
				{
					...
					{
						Roaduser r = (Roaduser)li.next();
						double pos = length - r.getPosition() - 2; /*EJUST: int-->double*/
						betaturnloop:
						{
							for(int t = 0; t <= turns.length; t++)
							{
	                            if(t < turns.length && pos >= 0)
								{
									if(turns[t].getPosition() >= pos)
									{
										...
										if(t==0)	// alpha->turns[0]
										{
											tp1 = new Point((int)(alphaCoord.x + 5 * Math.cos(alphaAngle)), (int)(alphaCoord.y - 5 * Math.sin(alphaAngle)));
											tp2 = turns[t].getCoord();
											ta1 = alphaAngle;
											ta2 = turns[t].getAngle() + Math.PI;
										}
								...
								{
	                              	Point tp1 = new Point((int)(alphaCoord.x - 5 * Math.cos(alphaAngle)), (int)(alphaCoord.y + 5 * Math.sin(alphaAngle)));
									Point p = avg(getCurvePoint(tp1,alphaAngle,1,n,x,y), getCurvePoint(tp1,alphaAngle,1,n+1,x,y));
...
// ======== OLD ROAD CODE ==================================================
	...
	for(int n = 1; n < width; n++)
	{
		...
		for(int n = 0; n < num_alpha_lanes; n++)
		{
			...
			{
				g.setColor(s.mayDrive() ? Color.green : Color.red);
        		g.fillOval((int)((x + sx2 * part2 + sx1 * (1-part2) - 3 - dx) * zf), (int)((y + sy2 * part2 + sy1 * (1-part2) - 3 - dy) * zf), 6,6);
			}
    	...
    	for(int n = 0; n < num_beta_lanes; n++)
		{
			...
			if(s != null && s.getType() == Sign.TRAFFICLIGHT)
			{
				...
				g.setColor(s.mayDrive() ? Color.green : Color.red);
				g.fillOval((int)((x + ex1 * part + ex2 * (1-part) - 3 - dx) * zf), (int)((y + ey1 * part + ey2 * (1-part) - 3 - dy) * zf), 6,6);
//				(int)(5 * zf),(int)(5 * zf));
     		}
