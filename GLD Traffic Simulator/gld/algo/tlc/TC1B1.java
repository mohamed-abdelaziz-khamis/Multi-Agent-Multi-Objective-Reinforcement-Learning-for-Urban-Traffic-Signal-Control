 ...
 * @version 0.915
 */
public class TC1B1 extends TCRL implements Colearning, InstantiationAssistant, Constants /*EJUST*/
{
	// TLC vars
	...
	*/
	 ...
	 */
	protected void relaxBucket(int laneId, double prevpos /*EJUST: int --> double*/) {
	...
	*/
					...
					PosMov[] posMovs, int penalty)
	{
		...
		for(int ti=0; ti<num_posmovs; ti++) {		// For All tl', pos'
			...
			if(p_index>=0) {	// There exists a situation in the P-table like the current one
				...
				R = rewardFunction(tlId, pos, curPMTlId, curPosMov.pos);			
					// Calculate the Rewardvalue, see if this option is in the possiblelanes&ranges
		...
		q_table[tlId][pos_int][desId][light?green_index:red_index]=newQvalue;
	}
