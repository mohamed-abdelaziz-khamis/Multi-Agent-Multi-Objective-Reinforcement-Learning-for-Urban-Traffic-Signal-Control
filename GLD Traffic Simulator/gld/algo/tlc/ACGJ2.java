 ...
 * 4. This algorithm has a keep green factor. This editable factor is an extra weight to keep the lights green, that were green in the previous timeStep.
 */
 
public class ACGJ2 extends TLController implements XMLSerializable,TwoStageLoader,InstantiationAssistant, Constants /*EJUST*/
{	
	...
	protected NextTimeSteps bestSoFar;
	...
	private class NextTimeSteps implements XMLSerializable
	{	int timeStepsForward;
		Infrastructure currentInfra;
		protected String myParentName="model.tlc";
		
		public NextTimeSteps ()
		{	
			currentInfra=infra;
		}
		
		// XMLSerializable implementation of NextTimeSteps
		
		public void load (XMLElement myElement,XMLLoader loader) throws XMLTreeException,IOException,XMLInvalidInputException
		{	
			timeStepsForward=myElement.getAttribute("timeSteps-forward").getIntValue();
		}
	
		public XMLElement saveSelf () throws XMLCannotSaveException
		{ 	
			XMLElement result=new XMLElement("nexttimeSteps");
			result.addAttribute(new XMLAttribute("timeSteps-forward",timeStepsForward));
	  		return result;
		}
  
		public void saveChilds (XMLSaver saver) throws XMLTreeException,IOException,XMLCannotSaveException
		{ 	
			// A NextTimeSteps object has no child objects
		}
	
		public String getXMLName ()
		{ 	
			return myParentName+".nexttimeSteps";
		}
		...
		public void calcPatValues(float patFactor)
		{
			...
			for (int i=1; i<temp.length; i++)
			{
				if (lastConfigs[i]!=null)
				{
					if (lastConfigs[0].signEquals(lastConfigs[i].signs))
					{
						...
						count++;
						totTimeSteps+=lastConfigs[i].numTimeSteps;
					}
					else
					temp[i]=null;
				}
			...
			if (lastConfigs[i]!=null)
			{
				if (lastConfigs[0].signEquals(lastConfigs[i].signs))
				{
					...
					if (lastConfigs[i].numTimeSteps>lastConfigs[0].numTimeSteps)
					{
						leftSecs = lastConfigs[i].numTimeSteps-lastConfigs[0].numTimeSteps;
						old = lastConfigs[i];
					}
					else 
					{
						old = lastConfigs[i-1];
						leftSecs = lastConfigs[i].numTimeSteps; 
					}
					...
					for (int j=0; j<dli.length; j++)
					{
						...
						for (int k=0; k<sgs.length; k++)
						{
							...
							{
								pat[dl.tldIndex]+=patFactor*(leftSecs/totTimeSteps);
							}
						}
					}
				}
	...
	private class DrivelaneInfo implements XMLSerializable,TwoStageLoader
	{
		...
		while (li.hasNext()) 
		{
			...
			if (ru_pos - pos > STOP_SPEED_MS*TIMESTEP_S /*EJUST replaced: ru_pos > pos */) break;
			else if (Math.abs(ru_pos - pos) <= STOP_SPEED_MS*TIMESTEP_S /*EJUST replaced: ru_pos == pos*/) {
				pos += ru.getLength();
				count++;
			}
		}
	...
	private class Configuration implements XMLSerializable
	{
		...
		public Configuration (Sign [] s, int numTimeSteps)
		{
			//this.signs     = signs;
		...
		public Configuration (boolean [] b, int numTimeSteps)
		{
			//this.signs     = signs;
		this.signs = b;
		this.numTimeSteps = numTimeSteps;			
		...
		public void load (XMLElement myElement,XMLLoader loader) throws XMLTreeException,IOException,XMLInvalidInputException
		{	numTimeSteps=myElement.getAttribute("num-timeSteps").getIntValue();
			signs=(boolean[])XMLArray.loadArray(this,loader);
		}
	
		public XMLElement saveSelf () throws XMLCannotSaveException
		{ 	XMLElement result=new XMLElement("config");
			result.addAttribute(new XMLAttribute("num-timeSteps",numTimeSteps));
	  		return result;
		}
	...
	{ 	if (NextTimeSteps.class.equals(request))
		{  return new NextTimeSteps();
		}
	...
	{ 	return NextTimeSteps.class.equals(request) ||
