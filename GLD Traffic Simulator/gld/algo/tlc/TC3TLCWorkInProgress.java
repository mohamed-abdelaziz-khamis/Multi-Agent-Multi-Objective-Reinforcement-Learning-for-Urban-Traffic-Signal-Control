 ...
 * @version 0.1
 */
public class TC3TLCWorkInProgress extends TCRL implements Colearning,InstantiationAssistant, Constants /*EJUST*/
{
	// TLC vars
	...
	*/
			...
			PosMov[] posMovs, Drivelane desired, int penalty)
	{
		...
		if(prevsign.getType()==Sign.TRAFFICLIGHT && (signnow.getType()==Sign.TRAFFICLIGHT || signnow.getType()==Sign.NO_SIGN)) {
			...
			recalcP(tlId, prevpos_int, desId, prevsign.mayDrive(), signnow.getId(), posnow_int, Ktl);			// OK
			recalcW(tlId, prevpos_int, desId);																// OK
			recalcV(tlId, prevpos_int, desId, prevsign.mayDrive(), Ktl, posMovs);							// OK
			recalcQa(tlId, prevpos_int, desId, prevsign.mayDrive(), signnow.getId(), posnow_int, posMovs, penalty);
			recalcQ(tlId, prevpos_int, desId, prevsign.mayDrive(), signnow.getId(), posnow_int, posMovs, Ktl, penalty);
		}
	}
	...
	protected void recalcQa(int tlId, double pos /*EJUST: int --> double*/, 
...
*/
		...
		if (tlNewId == tlId) {
			long noem = 0, deel = 0;
			CountEntry C = new CountEntry(tlId,pos_int,desId,light,tlId,posNew_int,-1);
			...
			W   = wTable[tlId][posNew_int][desId];
			Qa += P * (R+ (gamma * W));
		}


		qaTable[tlId][pos_int][desId][light?green_index:red_index] = Qa;
	}
