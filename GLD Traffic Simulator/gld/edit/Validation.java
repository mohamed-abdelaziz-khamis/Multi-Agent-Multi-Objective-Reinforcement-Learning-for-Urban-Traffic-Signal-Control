 ...
 * It checks whether there are any 'empty' nodes
        ...
        while (edgeNodes.hasMoreElements())
        {
            ...
            for (int j = 0; j < types.length; j++)
            {
                ...
                if (dests.length > 0)
                {
                	...
                	/** EJUST: Time Of Last Arrival of this ruType.*/
                	int timeStepOfLastArrival = -1;
                	
                	/** EJUST: The generated random number after the last spawned roaduser of type ruType*/
                	double lastGeneratedRandomNumber = 1;  
                    
                	/** EJUST: Weather condition*/
                	int weatherCondition = WeatherFactory.DRY;
                	
                	spawn[ruTypeCount] = new SpawnFrequency(types[j], freq, 
                    		distributionType /*EJUST*/, paramValue/*EJUST*/, 
                    		timeStepOfLastArrival/*EJUST*/, lastGeneratedRandomNumber /*EJUST*/,
    ...
    private Vector addIDs() throws InfraException
    {
        ...
        for (int i = 0; i < num_nodes; i++)
        {
            ...
            for (int j = 0; j < num_ibl; j++)
            {
                ...
                {
                    errors.add("ERROR: lane " + lane.getId() + " has sign == null");
                }
            }
        }
...
//                        }
    ...
    private Vector checkInfra() throws InfraException
    {
        ...
        {
            errors.addElement("ERROR: infrastructure should contain at least 2 special nodes.");
        }
        ...
        for (int i = 0; i < allNodes.length; i++)
        {
            ...
            {
                errors.addElement("ERROR: Node " + nd.getId() +  " doesn't have any road connected to it.");
            }
        }
    ...
    protected SpawnFrequency[] spawnFreq = {new SpawnFrequency(0, 0.0f, -1 /*EJUST*/, null /*EJUST*/, -1 /*EJUST*/, 1 /*EJUST*/, 0 /*EJUST*/), 
    										new SpawnFrequency(1, 0.0f, -1 /*EJUST*/, null /*EJUST*/, -1 /*EJUST*/, 1 /*EJUST*/, 0 /*EJUST*/)};
    ...
    private Vector checkSpawnFreqs() throws InfraException
    {
        ...
        for (int i = 0; i < edgnds.length; i++)
        {
            ...
            for (int j = 0; j < spfreq.length; j++)
            {
                if (spfreq[j].freq > 1)
                {
                    errors.addElement("ERROR: Edge-Node " + en.getId() + " Spawn-frequency >1 ");
                }
                if (spfreq[j].freq < 0)
                {
                    errors.addElement("ERROR: Edge-Node " + en.getId() + " Spawn-frequency <0 ");
                }
            }
        }
        return errors;
