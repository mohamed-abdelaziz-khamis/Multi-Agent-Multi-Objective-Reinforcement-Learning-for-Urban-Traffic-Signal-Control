 ...
 * @version 2.0
 */
public class TC1TLCOptHybridExplorationEnhancedMultiObjectiveBayesian extends TCColearnPerformanceIndex implements Colearning, InstantiationAssistant, HECinterface, Constants /*EJUST*/
{
	// TLC vars
	...
	protected final static String shortXMLName="tlc-tc1o1-hybrid-exploration-enhanced-multiobjective-bayesian";
	...
	public TC1TLCOptHybridExplorationEnhancedMultiObjectiveBayesian ( Infrastructure infra ) throws InfraException
	{	
		super(infra);
					...
					for (int j=0; j<dls.length; j++) 
					{
						...
						for (int k=0; k<num_pos_on_dl; k++)	
						{
							...
							for (int l=0; l<num_specialnodes;l++)	
							{
								...
								qTableMultiObjective[id][k][l][1]= 0.0f; //EJUST ICMLA 2012 version: 10.0f instead of 0.0f 
								...
								vTableMultiObjective[id][k][l]	= 0.0f; //EJUST ICMLA 2012 version: 5.0f instead of 0.0f								
	...
	public TLDecision[][] decideTLs() {
		...
		*/
		 ...
		 * 
		 * ICMLA 2012: A reasonable starting strategy for temperature is to start at 100 
		 * and decrease it by 1 every 10 iterations until you get to 1
		*/
		Kt = ( Kt > 1 && curTimeStep % 10 == 0 ) ? Kt-- : Kt;		
		
		/* random_chance = (float) Math.exp(-curTimeStep/Kt);	EJUST ICMLA 2012*/
		/* random_chance = (float) ((random_chance > 0.01) ? Math.exp(-0.005*curTimeStep): 0.01);	EJUST Samah 2010*/
							...
							double posNew /*EJUST: int --> double*/)
	{
		...
		try { /*POMDPGLD*/			
			...
			System.err.print("ArrayIndexOutOfBoundsException in: TC1TLCOptHybridExplorationEnhancedMultiObjectiveBayesian, r: 504, values: tlId: "+tlId+" pos: "+pos+" desId: "+desId);
			c_index = 0;
		}/*POMDPGLD*/
		...
		p_index = pTable[tlId][pos_int][desId].indexOf(thisChance); //returns -1 if the object is not found.
		...
		if(p_index >= 0)
			 ...
			 * because the average of the collected rewards when moving from the same current state s to the same next state s' will be fixed*/ 
			double R_AverageJunctionWaitingTime = rewardFunctionAverageJunctionWaitingTime(tlId, tlNewId);
			double R_FlowRate =  10 - R_AverageJunctionWaitingTime;
		...
		/*EJUST*/
		double r, r_AverageTripWaitingTime, r_AverageTripTime, r_AverageSpeed, r_Safety = 0, r_FuelConsumption, r_GreenWave=0, congestionFactor = 0;
		...
		if (tlNewId!=tlId) //L = Green
		{			
			/*EJUST: The congestion factor is independent from the new position but is function in the next lane congestion*/
			congestionFactor = congestionFactor(signNew.getLane());	
			 ...
			 * thus, the vehicle must minimize its speed either if it was in a primary or a secondary road,
			...
			if (checkStoppedVehicle(signNew.getLane(), E))												
				 ...
				 * (that means very slow speed "small distance travelled per time step" and thus higher safety), 
				 * then it will be penalized by the reward value (here the lowest penalty is -10).
				 * 
				 * Example: 
				 * If r_AverageSpeed = 0 --> r_Safety = -10
				 * If r_AverageSpeed = 1/3 --> r_Safety = -9
				 * If r_AverageSpeed = 1/2 --> r_Safety = -8 
				 * If r_AverageSpeed = 2 --> r_Safety = -2 
				 * If r_AverageSpeed = 3 --> r_Safety = -1  (highest penalty, lowest gain, and lowest chance to remain green)
				 *  
				 * Thus, when speed increases while the light is green --> the penalty increases that decreases the gain and leads to red light    
				 */
				r_Safety = 10/(Math.pow(r_AverageSpeed,2)+1); //Is multiplied below by (-1)			
			
			else if (!prevsign.getLane().isPrimary()) //Not a primary lane, i.e., partially residential and schools area
				/* EJUST: 
				 * Example: 
				 * If r_AverageSpeed = 0 --> r_Safety = -10
				 * If r_AverageSpeed = 1/3 --> r_Safety = -7.5
				 * If r_AverageSpeed = 1/2 --> r_Safety = -6.667 
				 * If r_AverageSpeed = 2 --> r_Safety = -3.333 
				 * If r_AverageSpeed = 3 --> r_Safety = -2.5  (high penalty, low gain, and low chance to remain green)
				 * */
				r_Safety = 10/(r_AverageSpeed+1); //Is multiplied below by (-1)				
		}
			...
			if (!prevsign.getLane().isPrimary()) //Not a primary lane, i.e., partially residential and schools area
				r_Safety = 10/(r_AverageSpeed+1); //Is checked below whether to be multiplied by (-1) or not			
		}
		...
		if (prevsign.getLane().isPrimary()){ //Primary lane, i.e., main road
			r_AverageTripTime = 10 * (1 - Math.pow(2, -Math.pow(r_AverageSpeed,2)));			
			r_FuelConsumption = (r_AverageSpeed/TIMESTEP_S) < (PRIMARY_ROAD_SPEED_LIMIT_MIN_KMH/3.6)?10: 
								// Very slow speed causes high fuel consumption, thus large penalty will be applied in case of red light to switch to green 
								// and lets the vehicle accelerates to enter the normal speed threshold 
				
								(r_AverageSpeed/TIMESTEP_S) > (PRIMARY_ROAD_SPEED_LIMIT_MAX_KMH/3.6)?-10:0;
								// Very high speed causes high fuel consumption, thus large negative penalty will be applied in case of green light to switch to red
								// and lets the vehicle decelerate to enter the normal speed threshold
		}
		else{ //Not a primary lane, i.e., partially residential and schools area
			r_AverageTripTime = 10 * (1 - Math.pow(2, -r_AverageSpeed));
			r_FuelConsumption = (r_AverageSpeed/TIMESTEP_S) < (SECONDARY_ROAD_SPEED_LIMIT_MIN_KMH/3.6)?10:
								(r_AverageSpeed/TIMESTEP_S) > (SECONDARY_ROAD_SPEED_LIMIT_MAX_KMH/3.6)?-10:0;
		}
		
		/* EJUST: Due to the Gain = Q(Red) - Q(Green) 
		 * thus, if the light is true (i.e., green) we multiply the reward by -1
		 */
		if (light){ 
			r_Safety *= -1;
			r_AverageTripTime *= -1;
		...
		if (prevsign.getLane().isPrimary() && light && platoonRoaduserCount >= 1 && platoonRoaduserCount <= MU) //Primary lane, i.e., main road
			r_GreenWave = -10; 
		...
		thisChance.addCongestionFactor(congestionFactor);		
	...
	{	
		...
		for(int t=0; t < num_p /*EJUST replaced: num_posmovs*/; t++) {		// For All tl', pos'				
			...
			if (P.tlId==tlId && P.pos==pos && P.desId==desId && P.light==light){	/*EJUST*/									
				...
				Q_MultiObjective += chance * (
						P.getAverageTripWaitingTimeReward()  + /*Cooperation better in transient and very worse in steady state*/
						P.getAverageTripTimeReward() + /*Cooperation better in transient and very worse in steady state*/
						P.getAverageJunctionWaitingTimeReward() + /*Cooperation better in transient and very worse in steady state*/
						P.getCongestionFactor() * P.getFlowRateReward() + /*Cooperation better in transient and steady state the same*/
						P.getGreenWaveReward() + /*Cooperation better in transient and steady state*/
						P.getSafetyReward() + /*Improper reward: Cooperation better in transient and steady state*/
						P.getFuelConsumptionReward() + /*Still remaining*/ 
		...
		//else 
		//{ 
		//	float prevNodePrevGain, prevNodeCurrGain, currNodePrevGain, currNodeCurrGain, prevNodeGainDifference, currNodeGainDifference, sum;
		//	Junction prevNode = (Junction)prevsign.getLane().getNodeComesFrom();
		//	prevNodePrevGain = prevNode.getPreviousGain();
		//	prevNodeCurrGain = prevNode.getCurrentGain();
		//	prevNodeGainDifference = Math.abs(prevNodeCurrGain-prevNodePrevGain);
			
		//	Junction currNode = (Junction)prevsign.getNode();
		//	currNodePrevGain = currNode.getPreviousGain();
		//	currNodeCurrGain = currNode.getCurrentGain();				
		//	currNodeGainDifference = Math.abs(currNodeCurrGain-currNodePrevGain);
			
			/* alpha: Learning/cooperation rate used to fasten the learning process (i.e., Q-values reach faster to steady state)
			 * 		 Initially has large value (~1) at transient period Q = Q_transferred 
			 * 		 and decreases by time (~0) at steady state Q = Q_MultiObjective 
			 * 
			 * There are three cases:
			 * 1- At the normal steady state where: 
			 * + the prev junction that reaches its steady state does not have upnormal conditions to be transferred 
			 * (in general at steady state no more knowledge to be transferred)
			 * + the current junction that reaches its steady state has no problem to learn from the previous node
			 * --> In this case: alpha ~ 0
			 *  
			 * 2- When the currNode is in a transient state, then decrease the in-cooperation (i.e., from the prevNode)
			 * --> And in this case: alpha decreases as prevNodeGainDifference decreases.
			 * 
			 * 3- When the prevNode is in a transient state, then increase the out-cooperation (i.e., to the currentNode)
			 * --> And in this case: alpha increases as currNodeGainDifference decreases.
			 * 
			 */
			
		//	sum = prevNodeGainDifference + currNodeGainDifference;  
		//	if (prevNodeGainDifference < THRESHOLD*prevNodeCurrGain && currNodeGainDifference < THRESHOLD*currNodeCurrGain)
				/*hightly steady state case*/
		//		alpha  = 0.01f;
		//	else 
		//		if (sum == 0) /*hightly transient case*/{
				/* 
				 * A junction of totalGain = 0 either due to an "initial" Q_Red and Q_Green for all possible traffic light configurations equal zero
				 * Or for example 7 traffic light configurations each has gain equals -10 (i.e., Q_Red < Q_Green) 
				 * and one traffic light configuration has gain equals 70 (i.e., Q_Red > Q_Green)
				 * Those two cases are considered as hightly transient 
				 * (as for sure the 7 traffic light configurations that are red --> Q_Red will gradually increase till exceeds Q_Green) 
				 */
		//		alpha  = 1;
		//	}			
		//	else /*hybrid case*/	
		//		alpha = prevNodeGainDifference/sum;
			
			/* Note that: the case of tlId == prevSign cannot happen, 
			 * because we set the prevSign value after calling Update_Q_Value
			 * It is wrong to set the prevSign by tlId (i.e., the sign before crossing) before calling Update_Q_Value  
			 * because both Q-functions will refer to the same thing: Q_MultiObjective and Q_transferred
			 */			
		//	float Q_transferred = qTableMultiObjective[prevSign][prevSignLastPosition][desId][light?green_index:red_index];
	...
	/**EJUST comment: 
	 * Co-learning ATWT/AJWT performance index reward function
	...
	{						
		...
		//EJUST ICMLA 2012 version: 10 instead of 1
		//L = Red	
		return 10; //tlId==tlNewId and pos-posNew <= STOP_SPEED_MS*TIMESTEP_S 	
	...
	protected int rewardFunctionAverageJunctionWaitingTime(int tlId, int tlNewId) {		
		if (tlId==tlNewId) //Junction waiting time will increase due to the current lane has red light or congestion
			return 10; // tl=tl’ regardless the pos		
	...
	{
		if(b) c.setStatus("Using HEC on TC1TLCOptHybridExplorationEnhancedMultiObjectiveBayesian");
		else c.setStatus("Using TC1TLCOptHybridExplorationEnhancedMultiObjectiveBayesian without HEC");
		hecAddon = b;
	}
	...
	public class PEntry implements XMLSerializable, Serializable
	{
		...
		double  R, R_AverageTripWaitingTime, R_AverageTripTime, R_AverageSpeed, R_AverageJunctionWaitingTime, 
				R_FlowRate, R_Safety, R_GreenWave, R_FuelConsumption, congestionFactor;
		...
		/*EJUST*/
		public void addCongestionFactor(double _congestionFactor){
			congestionFactor += _congestionFactor;
		}
		...
		public double getCongestionFactor(){
			return congestionFactor/getSameSituation();
		}
	...
	{ 	
		System.out.println("Called TC1TLC-opt Hybrid Exploration Enhanced Multi-objective Bayesian instantiation assistant ??");
	...
	{ 	
		...
		{ 
			throw new ClassNotFoundException("TC1 Hybrid Exploration Enhanced Multi-objective Bayesian IntstantiationAssistant cannot make instances of "+ request);
		}
	}
}